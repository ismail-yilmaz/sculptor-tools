	...
	auto DrawBand = [&](int c) {
		...
		auto DrawPrisms = [&](const Vector<int>& list) {
			for(int i = 0; i < list.GetCount(); i++) {
				...
				float px0 = prim.screenpos[0].y;
				float px1 = prim.screenpos[1].y;
				float px2 = prim.screenpos[2].y;

				float minpx = min(px0, min(px1, px2));
				float maxpx = max(px0, max(px1, px2));

				int miny = max(0, (int) floor(minpx));
				int maxy = min(size.cy - 1, (int) ceil(maxpx));
...
void Sculptor::RasterizeTriangle(const Primitive& prim, Size size, int miny, int maxy)
{
