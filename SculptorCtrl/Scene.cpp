...
void GLScene::Render(ModelGL& model)
{
    ...
    glPolygonMode(GL_FRONT_AND_BACK, glpipeline.wireframe ? GL_LINE : GL_FILL);
    ...
    for(int i = 0; i < lightcount; i++) {
        String dirname = Format("uLightDir[%d]", i);
        String colname = Format("uLightCol[%d]", i);
        glUniform3f(glGetUniformLocation(prog, ~dirname), glpipeline.lights[i].dir.x, glpipeline.lights[i].dir.y, glpipeline.lights[i].dir.z);
        glUniform4f(glGetUniformLocation(prog, ~colname), glpipeline.lights[i].color.r / 255.f, glpipeline.lights[i].color.g / 255.f, glpipeline.lights[i].color.b / 255.f, 1.0f);
    }
