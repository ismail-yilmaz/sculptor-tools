...
void LabyrinthScene::Render(GLScene& gls)
{
	Sizef sz = GetSize();
	if(sz.cy == 0)
		return;
	...
	gls.LookAt(camera, Point3D(0, 0, 0), Point3D(0, 0, 1));

	gls.ClearLights();
	gls.AddLight(Point3D(-5, -10, 15), White());
...
void LabyrinthScene::MouseWheel(Point p, int zdelta, dword keyflags)
{
	camdist = clamp(camdist - zdelta * 0.005f, 10.0f, 50.0f);
	Refresh();
}

void LabyrinthScene::LeftDown(Point p, dword keyflags)
{
	mousedrag = true;
	mousep = p;
	SetCapture();
...
void LabyrinthScene::LeftUp(Point p, dword keyflags)
{
	mousedrag = false;
	ReleaseCapture();
}

void LabyrinthScene::MouseMove(Point p, dword keyflags)
{
	if(mousedrag) {
...
{
	TopWindow app;
	app.Title("Labyrinth Game (OpenGL version, using SculptorCtrl)");
	app.Sizeable().Zoomable().CenterScreen().SetRect(0, 0, 1024, 768);

	LabyrinthScene gamescene;
	app.Add(gamescene.SizePos());
	app.Run();
}